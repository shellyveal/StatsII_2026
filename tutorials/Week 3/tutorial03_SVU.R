...
results_binom <- optim(fn=binom_likelihood, outcome=y_binom, input=X, 
                       par=c(0,1), hessian=T, method="BFGS")
...
# print our estimated coefficients (intercept and beta_1)
results_norm <- optim(fn=norm_likelihood, outcome=y_norm, input=cbind(1, X), 
                      par=c(1,1,1), hessian=T, method="BFGS")
results_norm2 <- optim(fn=norm_likelihood, outcome=y_norm, input=cbind(1, X), 
                       par=c(1,1,1), hessian=T, method="BFGS")
# print our estimated coefficients (intercept and beta_1)
...
lb <- exp(real_beta0 + real_beta1 * X)
y_pois <- rpois(100, lb)
...
pois_likelihood <- function(parameter, outcome, input) {
  lambda <- exp(parameter[1] + parameter[2]*input)
  -sum(dpois(outcome, lambda, log = TRUE))
}

results_pois <- optim(par = c(0,1), fn = pois_likelihood,
                      outcome = y_pois, input = X,
                      hessian = TRUE, method = "BFGS")

pois_likelihood_2 <- function(parameter, outcome, input) {
  eta <- input %*% parameter
  lambda <- exp(eta)
  -sum(dpois(outcome, lambda, log = TRUE))
}
